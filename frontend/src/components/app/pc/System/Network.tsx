...
const Network: React.FC<{}> = () => {
  ...
  return (
    <Stack spacing={2}>
      <FormItem title={lang('网址解析', 'URL Parsing')}>
        <TextField
          placeholder={lang(
            '留空则使用默认服务',
      ...
      <FormItem title={lang('网址解析超时', 'Parsing Timeout')}>
        <TextField
          placeholder=""
          fullWidth
      ...
      <FormItem title={lang('搜索引擎地址', 'Search Engine')}>
        <TextField
          placeholder="https://www.baidu.com/s?wd=TEXT"
          fullWidth
          error={
            !z
              .union([z.string().url().includes('TEXT'), z.literal('')])
              .safeParse(settings.searchUrl).success
          }
          value={settings.searchUrl}
          onChange={(e) => {
            const v = e.target.value;
            changeStore((d) => {
              d.systemForm.settings.searchUrl = v;
            });
          }}
        />
      </FormItem>
